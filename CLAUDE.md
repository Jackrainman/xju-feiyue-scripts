...
软件开发实验室（XjuSelab）的 Tampermonkey 用户脚本 monorepo。三个脚本 + 一个共享后端，统一通过 `feiyue.selab.top` 分发。
...
> 飞跃·导入 Importer 的**脚本本体已在本仓**（`scripts/feiyue-importer/`）。其**后端中转端点** `/notes/transcript-stash` 与**导入向导**仍属 [Aurash](https://github.com/winbeau/Aurash)（飞跃 web 应用）——改脚本时留意两边联动（安装检测标记 `window.__feiyueImporterReady` / `data-feiyue-importer`、向导安装链接 `IMPORT_USERJS_URL`），详见 `scripts/feiyue-importer/README.md`。
